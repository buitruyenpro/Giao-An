<?php 
	...
	$headerNameParent	= $aliasName['controller'][$this->arrParams['controller']];
	$headerNameChild	= $aliasName['action'][$this->arrParams['action']];
	
	$xhtmlHeader	= sprintf('<h1>%s <small>%s</small></h1>', $headerNameParent, $headerNameChild);
	
	$xhtmlBreadcrumb		= '';
	if($this->arrParams['controller'] != 'index'){
		if($this->arrParams['action'] == 'index'){
